...
function Products() {
    ...
    function getListSale() {
        axios.get('https://localhost:44382/api/ProductSale')
            .then((res) => {
    ...
    function getListProduct() {
        axios.get('https://localhost:44382/api/UserProduct')
            .then((res) => {
                const myRespo = res.data;
    ...
    function handleAddCart(e,idproduct) {
        e.preventDefault();
    ...
    return (
                            ...
                            <ul className="products columns-3">
                                {lsSale.map((s, index) => (
                                    <li key={index} className="product type-product post-3074 instock last status-publish instock product_cat-coffee-beans product_tag-coffee has-post-thumbnail sale shipping-taxable purchasable product-type-simple">
                                        <Link to={`/ViewProduct/${s.id}`} className="woocommerce-LoopProduct-link woocommerce-loop-product__link">
		...
		c0.7-1.3,2-2.2,2.6-3.6C57.8,35.7,56.1,33.2,56.5,29.7z" fill="#ffffff"></path>
				...
				c0,0.3,0.1,0.6,0.4,0.9c0.2,0.2,0.6,0.4,0.9,0.4l5,0c0.7,0,1.2-0.6,1.2-1.2C48.8,34.3,48.2,33.8,47.5,33.8z"></path>
                                        ...
                                        </Link><a href="/AddToCart" onClick={(e) => handleAddCart(e,s.idproduct)} data-quantity="1" className="button wp-element-button product_type_simple add_to_cart_button ajax_add_to_cart" data-product_id="3074" data-product_sku="TBMW-001-008-6" aria-label="Add “Arabica Roasted Beans” to your cart" rel="nofollow">Add to cart</a></li>
                                ))}
                                {lsProduct.map((p, index) => (
                                    <li key={index} className="product type-product post-3048 status-publish instock last product_cat-coffee-beans product_tag-coffee product_tag-sweet has-post-thumbnail shipping-taxable purchasable product-type-simple">
                                        ...
                                        </Link><a href="/AddToCart" onClick={(e) => handleAddCart(e,p.id)} data-quantity="1" className="button wp-element-button product_type_simple add_to_cart_button ajax_add_to_cart" data-product_id="3048" data-product_sku="SBMW-001-009-4" aria-label="Add “Ethiopia Roasted Beans” to your cart" rel="nofollow">Add to cart</a></li>
                                ))}
                            </ul>
                        ...
                        </ul></div></div><div className="widget woocommerce widget_recent_reviews"><div className="widget-content"><h4 className="widget-title">Recent reviews</h4><ul className="product_list_widget">
