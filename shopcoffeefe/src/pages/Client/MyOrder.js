...
function MyOrder() {
    ...
    function getListOrder() {
        ...
        axios.get(`https://localhost:44382/api/MyOrder/${id}`)
            .then((res) => {
                setTotal(0);
    ...
    const sendPostRequest = async (myOrder) => {
        try {
            const resp = await axios.put('https://localhost:44382/api/MyOrder', myOrder);
            getListOrder();
        } catch (err) {
            // Handle Error Here          
        }
    ...
    const sendDeleteRequest = async (idOrder) => {
        try {
            const resp = await axios.delete(`https://localhost:44382/api/MyOrder/${idOrder}`);
            getListOrder()
        } catch (err) {
            // Handle Error Here          
        }
    ...
    function handleDeleteOrder(e,idOrder) {
        e.preventDefault();
        sendDeleteRequest(idOrder);
    }
                                            ...
                                            lsOrder.length !== 0
                                                ...
                                                lsOrder.map((c, index) => (
                                                    ...
                                                    <tr className="woocommerce-cart-form__cart-item cart_item" key={index}>
                                                        ...
                                                        <td className="product-remove">
                                                            <a className="remove container" onClick={(e)=>handleDeleteOrder(e,c.id)} data-ajax="true" data-ajax-mode="replace" data-ajax-update="#divPart" href="/Cart/removeCart?idCart=@item.id">&times;</a>
                                                        </td>
